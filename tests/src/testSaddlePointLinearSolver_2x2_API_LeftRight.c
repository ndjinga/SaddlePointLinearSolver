...
/*            Use an upper block triangular matrix U and  a lower block triangular matrix L and perform the change of variables  X_hat = U^{-1}X, A_hat = L*A_{input}*U */
/*            Solve the transformed system A_{hat}*X_{hat} = b_{hat} with b_hat = L*b_input, A_hat = L*A_{input}*U */
...
int main( int argc, char **args ){
	...
	Mat A_input, A_hat, Pmat, C_hat, G_hat, D_hat, C_hat2, diag_2M;
	Mat M, G, D, C;
	IS is_U,is_P;
	Vec b_input, b_hat, X_hat, X_anal;
	Vec v;
...
//##### Calling KSP solver and monitor convergence
    ...
    solveLeftRightTransformedSystemForXhat( A_hat, Pmat, is_U, is_P, b_hat, &X_hat, rtol,PETSC_DEFAULT,PETSC_DEFAULT, PETSC_DEFAULT, &residu);
